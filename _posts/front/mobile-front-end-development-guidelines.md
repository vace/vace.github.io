...
title: "移动端前端踩坑攻略"
date: 2016-02-01
summary: "汇总移动前端开发过程中各种兼容性问题及解决方案，包括样式布局、交互体验、UI优化等常见坑点，针对iOS和Android的实用技巧集锦。"
tags: [前端, H5, CSS]
---

## 概述

本文档针对移动前端开发，包括 `Hybrid` 应用中的web页面，非 `Native` 应用开发。文中总结了移动端开发中常见的兼容性问题和解决方案，帮助开发者快速定位和修复问题。
...
* [边框圆角致背景溢出](#background-overflow)
* [一个失败的圆（圆角）](#border-radius-percentage)
...
* [设置添加到主屏幕的Web App标题](#shortcut-title)
* [设置添加到主屏幕的Web App图标](#shortcut-icon)
* [添加到主屏幕时隐藏地址栏和状态栏（即全屏）](#hide-bar)
* [添加到主屏幕时设置系统顶栏颜色](#status-bar-style)
* [电话号码识别](#tel)
* [邮箱地址识别](#email)
...
> ⚠️ 对于上述无法裁减边框之外背景的手机，将值定义为 `padding-box | content-box` 都能修复这问题，更推荐使用 `padding-box`。因为使用 `content-box`，如果定义了 `padding` 不为 `0`，背景将无法铺满元素。
...
> **最佳实践**：如果你觉得这样定义不够灵活，想懒一点，那么其实可以给 `border-radius` 预设一个比较大的值，比如 `100rem`，用以避免当元素的尺寸变了，圆角半径也得跟着变，除非元素的尺寸超出了你预设的阀值。
...
如果你是从`PC`开发转到移动平台的，或者应该记得在`PC`端，`Chrome`及后来加入Webkit阵营的`Opera`都不支持页面字号小于`12px`，当然你可以通过更改浏览器设置来改变这一情况，然而这并没有什么卵用，不是么？
...
> ⚠️ 如果希望你的程序足够安全，尽量不要定义小于12px的字号，或者换一种方式来实现。

> **实现原理**：假设你的项目使用了`rem`，那么不要使用`10`作为换算因子，原因也如上所述。
...
> **最佳实践**：如果你想要网页标题和App名字不一样的话，那就只有iOS才行。
...
> **最佳实践**：实际情况下，大部分智能手机都接近或者已经达到视网膜屏质量，所以如果想省事的话，可以分别为 `iPhone` 和 `iPad` 定义一种高质量的 `icon` 即可。
